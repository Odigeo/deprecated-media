...
Ocean is an architecture for creating agile server oriented architectures (SOAs).
...
Please also refer to OceanFront, the cross-platform Javascript front end browser client library.
...
Ocean requires Ruby 2.0 and Ruby on Rails 3.2.13 or later.
...
To create an Ocean resource based on an SQL model, do the following:
...
Next, look at +config/application.rb+. Locate the following two lines:
...
If the second line is commented out, remove the comment so the line reads:

 # Register observers for BANning, etc. Uncomment when you have a model to observe.
 config.active_record.observers = [:basic_ban_observer]

Now open +app/observers/basic_ban_observer.rb+. If the contents is commented out, uncomment it.

Next, locate the following line:

 observe :foo_model, :bar_model, :baz_model

Modify it to include the name of your resource:

 observe :quux

This ensures that your resource can be aggressively cached, and that modifications to member resources will invalidate cached objects as necessary. 

If you at a later point create another Ocean resource, simply tack it on to the end of the list. For instance,

 observer :quux, :thingy
